import { QUESTIONS, MESSAGES, ROOMS, initialState, transition, currentBooking, roomPrice, nights, addDays, isoDay } from './state.js';

const app = document.querySelector('#app');
const STORAGE = 'tl-chat-prototype-v1';
let state;
...
function chatMarkup() {
  ...
  return `${header()}<main class="messages" role="log" aria-label="История переписки" aria-live="polite" aria-relevant="additions"><div class="day-label">Сегодня</div>${state.chat.map(m=>{
    if(m.who==='notice')return `<div class="manager-notice">${MESSAGES[m.key]}</div>`;
    const guest=m.who==='guest';
    return `<div class="message-row ${guest?'outgoing':'incoming'}"><div class="bubble ${guest?'guest':'bot'}">${guest?QUESTIONS[m.key]:MESSAGES[m.key]}</div>${m.key==='booking'&&!guest?'<button class="booking-cta" data-action="open">Перейти к бронированию</button>':''}</div>`;
...
function roomCard(key) {
  ...
  return `<article class="room-card"><div class="room-image"><img src="${asset(r.photo)}" alt="${r.name}" class="room-photo"><span class="bestseller">${icon('context0-imgBficCup2424.svg')}Лидер продаж</span><span class="gallery-dots" aria-hidden="true">● · · ·</span></div><div class="room-content"><h2>${r.name}</h2><div class="properties"><span>${icon('context0-imgBfic3People2424.svg')}до ${r.places} мест</span><span>${icon('context0-imgBficSquare2424.svg')}${r.area} м²</span><span>${icon('context0-imgBeicQuantityRoom24.svg')}${r.rooms} комн.</span></div><div class="room-price"><small>от</small> ${money(roomPrice(state,key))} ₽<small class="price-detail">${nightLabel(nights(b))} / ${guestLabel(b)}</small></div><button class="primary" data-action="room" data-room="${key}" aria-label="Выбрать ${r.name}">Выбрать</button></div></article>`;
}
function roomMarkup() {
  const b=currentBooking(state);
  return `<div class="mini-scroll" data-scroll="room"><section class="booking-controls"><div class="promo-row"><span>${icon('context0-imgBficLock2424.svg')}Гарантия лучшей цены</span>${icon('context0-imgAva.svg')}</div><button class="select" data-action="dates" aria-label="Выбрать даты"><span>${dateRange(b)}</span>${icon('context0-imgIcons18IcArrow18Down.svg')}</button><button class="select" data-action="guests" aria-label="Выбрать гостей"><span>${guestLabel(b)}</span>${icon('context0-imgIcons18IcArrow18Down1.svg')}</button></section><h1 class="step-title">Выберите номер</h1><div class="cards">${roomCard('standard')}${roomCard('luxe')}</div></div>`;
}
function rateMarkup() {
  const r=ROOMS[state.selected||'standard'],b=currentBooking(state),count=nights(b);
  return `<div class="mini-scroll" data-scroll="rate"><div class="rate-title"><button class="icon-button" data-action="back" aria-label="Назад к номерам">${icon('context1-imgBficArrowLeft2424.svg')}</button><h1>Выберите тариф</h1></div><div class="cards rate-cards"><article class="selected-room"><img src="${asset(r.photo)}" alt="${r.name}"><div><h2>${r.name}</h2><p>До ${r.places} мест · ${r.area} м²<br>${r.rooms} ${r.rooms===1?'комната':'комнаты'}</p></div></article>${['Гостиничный','Выгодный'].map((title,i)=>`<article class="rate-card"><h2>Тариф ${title.toLowerCase()}</h2><ul class="benefits"><li>${icon(i?'context1-imgBficHb2424.svg':'context1-imgBficFood2424.svg')}<span>${i?'Полный пансион':'Завтрак'}</span></li><li>${icon('context1-imgBficCancellationRule2424.svg')}<span>Бесплатная отмена до ${dateText(addDays(b.start,-2))}, 14:00</span></li><li>${icon('context1-imgBficPayment2424.svg')}<span>Без предоплаты</span></li><li>${icon('context1-imgBficGift2424.svg')}<span>Бассейн, Парковка, Бильярд, Настольный теннис, Прокат велосипедов</span></li></ul><div class="rate-footer"><p>Стоимость за <b>${nightLabel(count)}</b><small>Налоги включены</small></p><div><strong>${money(roomPrice(state,state.selected||'standard')+i*5000)} ₽</strong><button class="primary" data-action="rate" aria-label="Выбрать тариф ${title.toLowerCase()}">Выбрать</button></div></div></article>`).join('')}</div></div>`;
}
